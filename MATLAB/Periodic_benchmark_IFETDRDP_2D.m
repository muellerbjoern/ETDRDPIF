function [runtime,u_soln,u_ex] = Periodic_benchmark_IFETDRDP_2D(dt,steps,do_plot)
...
%#d = 0.1;
d=1.0;
Adv = a/2.0*ones(num_species, dim);
Diff = d/2.0*ones(num_species, dim);
...
Uex = reshape(Uex, steps, steps);
u_ex = Uex;
Vex = reshape(Vex, steps, steps);
