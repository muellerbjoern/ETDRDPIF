...
def benchmark_simple(dt, steps, a=1, out=False):
    ...
    if out:
        ...
        plt.savefig(f'cfl_fouriermode_C{a*dt*steps}_a{a}_h{1/steps}_dt{dt}_te{te}.eps')
        plt.show()
        plt.clf()
...
if __name__ == "__main__":
    ...
    for i, a in enumerate(a_vals[::-1]):
        ...
        for j, h_exp in enumerate(range(n)):
            ...
            sys.stdout.flush()
    sys.stdout.flush()
    np.save("cfl_fouriermode_central_errors_euclid.npy", errors_euclid)
    np.save("cfl_fouriermode_central_errors_max.npy", errors_max)
    np.save("cfl_fouriermode_central_orders_euclid.npy", orders_euclid)
    np.save("cfl_fouriermode_central_runtimes.npy", runtimes)
    np.save("cfl_fouriermode_central_params_ahk.npy", params)
